	...
	<div class="col-sm-12 col-xl-12">
	 ...
	 <!-- <b-form>
	  ...
	  <li>
	     ...
	     <li><button class="btn btn-primary check-btn" type="button" @click="assignTelemarketers()">{{ $t('assign_telemarketers') }}</button></li>
     </ul>
    </div>
</div>
...
<!--  <div class="col-sm-12 col-xl-12">
  ...
  <!--  <div class="custom-sel-outer">
   ...
   <table class="table table-striped custom-table">
          ...
          <tbody>
            <tr v-for="(tel, index) in telemarketers"> 
	            <td> 
	            	<div class="custom-control custom-checkbox">
					    <input type="checkbox" class="custom-control-input"  :value=tel.id :id="index"  @click="selectTelemarketer($event)" v-model="selectedTelemarketers">
 ...
 import axios from 'axios';
  ...
  export default {
    ...
    mounted() {
        ...
        this.fetchTelemarketers(1);
        this.fetchSupervisors();
    },
   
    methods: {
      selectSupervisor(){
      	this.selectedTelemarketers = [];
      },		
      fetchTelemarketers(page) {
          ...
          var filters = {
	        ...
	        this.$axios.get('telemarketers?'+queryString, {
              params: {
                page: page,
            }
	        }).then(response => {
	          // JSON responses are automatically parsed.
	          this.telemarketers = response.data.data;
	    ...
	    fetchSupervisors(ctx, callback)
	     ...
	     .then(response => {
	      ...
	      .catch(e => {
   		...
   		selectTelemarketer (e) {
		    if (e.target.checked) {
		      	//Get selected Ids
		        var salesmanID = e.target.value
		        
		        this.selectedTelemarketers.push(salesmanID) 
		    }
		    else {
		     	//Removed unchecked Ids
		     	 for (var i = 0; i < this.selectedTelemarketers.length; i++ ) {
		          if (this.selectedTelemarketers[i] == e.target.value) {
		            this.selectedTelemarketers.splice(i, 1)
		          }
		        }
		    }
	 	},
	 	...
	 	selectAllChk() {
			this.selectedTelemarketers = [];
			if (!this.selectAll) {
				for (let i in this.telemarketers) {
					this.selected.push(this.telemarketers[i].id);
					this.selectedTelemarketers.push(this.telemarketers[i].id);
				}
			}
		},
	    async assignTelemarketers(customers)
	    {
	    	...
	    	if( this.selectedTelemarketers.length == 0)
	    	{
	    		alert("Please Select Telemarketering")
	    		return false;
	    	}

	    	var formInput = {
	          'user_id': this.selectedSupervisor.id,
	          'telemarketers': this.selectedTelemarketers
	        }
	        var salesmaName = this.selectedSupervisor.name
	        //Post data
         
	    	await this.$axios.post('supervisors/assign/telemarketers',formInput)
