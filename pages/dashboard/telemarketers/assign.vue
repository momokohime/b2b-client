  ...
  <b-form>
    ...
    <li>
       <select v-model="selectedSalesman">
        <option value="none">{{ $t('select_tk') }}</option>
 ...
 import axios from 'axios';
  ...
  export default {
    ...
    methods: {
      ...
      fetchTelemarketers(ctx, callback)
       {
        this.$axios.get(`telemarketers`)
       .then(response => {
          // JSON responses are automatically parsed.
         this.salesmen = response.data.data;
    ...
    selectAllChk() {
      ...
      async assignCustomers(customers)
      {
        ...
        var formInput = {
          ...
          //Post data
        await this.$axios.post('telemarketers/assign',formInput)
