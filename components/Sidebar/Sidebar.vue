                ...
                <template v-for="(childL1) in item.children">
                  ...
                  <template v-else>
                    ...
                    <SidebarNavItem :classes="item.class">
                      <SidebarNavLink :name="childL1.name" :url="childL1.url" :icon="childL1.icon" :badge="childL1.badge" :variant="item.variant" id="foo-baar"/>
                    </SidebarNavItem>
                    <!-- eslint-enable -->
                  </template>
                </template>
...
export default {
  ...
  watch: {
    $route () {
      setTimeout(function(){ 
           var x = document.querySelectorAll(".nav-item.nav-dropdown.nuxt-link-active")
